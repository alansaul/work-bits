...
def plot_model_with_samples(m, samples_label='Realizations of $f(x)$'):
    ...
    ax.plot(Xpred, f_samples[:,:-1], c=colors[1], alpha=alpha)
    ax.plot(Xpred, f_samples[:,-1], c=colors[1], alpha=alpha, label=samples_label)
...
intensities = fs + 4
Yp = np.array([[sp.random.poisson(intensity) for intensity in intensities]]).T
Y = np.array([[sp.random.normal(f, scale=1.0) for f in fs]]).T
...
if bernoulli:
    ...
    ax_bern_f.plot(Xpred, f_samples[:,:-1], c=colors[0], alpha=alpha)
    ax_bern_f.plot(Xpred, f_samples[:,-1], c=colors[0], alpha=alpha, label='Realizations of $f(x)$')
    ax_bern_f.set_xlim(Xpred.min(), Xpred.max())

    ax_bern_link_f.plot(Xpred, link_f_samples[:,:-1], c=colors[1], alpha=alpha)
    ax_bern_link_f.plot(Xpred, link_f_samples[:,-1], c=colors[1], alpha=alpha, label='Realizations of $\Phi(f(x))$')
    ...
    fig_bern.savefig('BernoulliPosterior.pdf', **plot_kw)
...
if gp:
    ...
    fig_gp.savefig('PosteriorGP.pdf', **plot_kw)
