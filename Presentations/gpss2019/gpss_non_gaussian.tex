...
\frame{
    ...
    \begin{minipage}[t][3cm][t]{\textwidth}
        \begin{align*}
            \only<1>{p(\fV|\yV) &= \frac{1}{{\color{intract}Z}}h(\fV)\\}%
            \only<2-3>{\log p(\fV|\yV) &= \log \frac{1}{{\color{intract}Z}} + \log h(\fV)\\}%
            \only<3-4>{&\approx \log \frac{1}{{\color{intract}Z}} + \log h(\aV) + \frac{d\log h(\aV)}{d\aV}(\fV - \aV)\\%
            &+ \frac{1}{2}(\fV - \aV)^\top\frac{d^{2}\log h(\aV)}{d\aV^{2}}(\fV - \aV) + \cdots}
            \only<5>{\log p(\fV|\yV) &\approx \log \frac{1}{{\color{intract}Z}} + \log h(\hat{\fV}) + \frac{d\log h(\hat{\fV})}{d\hat{\fV}}(\fV - \hat{\fV})\\}%
            \only<6>{\log p(\fV|\yV) &\approx \log \frac{1}{{\color{intract}Z}} + \log h(\hat{\fV}) + \frac{d\log h(\hat{\fV})}{d\hat{\fV}}(\fV - \hat{\fV})\\%
            &+ \frac{1}{2}(\fV - \hat{\fV})^\top\frac{d^{2}\log h(\hat{\fV})}{d\hat{\fV}^{2}}(\fV - \hat{\fV})}
            \only<7-8>{\log p(\fV|\yV) &\approx \log \frac{1}{{\color{intract}Z}} + \log h(\hat{\fV}) + \frac{d\log h(\hat{\fV})}{d\hat{\fV}}(\fV - \hat{\fV})\\%
            &+ \frac{1}{2}(\fV - \hat{\fV})^\top\frac{d^{2}\log h(\hat{\fV})}{d\hat{\fV}^{2}}(\fV - \hat{\fV}) + \cdots\\}%
            \only<9>{\log p(\fV|\yV) &\approx \log \frac{1}{{\color{intract}Z}} + \log h(\hat{\fV}) + \frac{d\log h(\hat{\fV})}{d\hat{\fV}}(\fV - \hat{\fV})\\%
            &+ \frac{1}{2}(\fV - \hat{\fV})^\top\frac{d^{2}\log h(\hat{\fV})}{d\hat{\fV}^{2}}(\fV - \hat{\fV})\\}%
            \only<10-11>{p(\fV|\yV) &\approx \frac{1}{{\color{intract}Z}}h(\hat{\fV})\exp\left\{-\frac{1}{2}(\fV - \hat{\fV})^{\top}\left(-\frac{d^{2}\log h(\hat{\fV})}{d\hat{\fV}^{2}}\right)(\fV - \hat{\fV})\right\}\\}%
            \only<11>{&= \gaussianDist{\fV}{\hat{\fV}}{\left(-\frac{d^{2}\log h(\hat{\fV})}{d\hat{\fV}^{2}}\right)^{-1}}}%
        \end{align*}
    ...
    \begin{minipage}[t][2cm][t]{\textwidth}
        ...
        \only<9>{\centering \includegraphics[height=3.5cm,keepaspectratio]{../diagrams/taylor/log_hf_taylor_2_only.pdf}}%
        \only<10>{\centering \includegraphics[height=3.5cm,keepaspectratio]{../diagrams/taylor/exp_approx.pdf}}%
        \only<11>{\centering \includegraphics[height=3.5cm,keepaspectratio]{../diagrams/taylor/norm_approx.pdf}}%
        \only<4>{%
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Uses an iterative algorithm to update $t_{i}$'s, such that the approximate marginal moments of $q(\fV_{i})$, match the marginal moments if $t_{i}$ was replaced with the true likelihood $p(\yV_{i}|\fV_{i})$. 
