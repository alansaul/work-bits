...
\frame{
    ...
    \begin{itemize}
        ...
        \item Can normalise easily with numerical integration, $\int p(y_1=1|f_1)p(f_1) df_1$.
    \end{itemize}
    \begin{overprint}
...
\frame{
    \frametitle{Non-Gaussian posteriors illustrated}
    \begin{itemize}
        \item Now consider two observations, $y_{1} = 1$ and $y_{2} = 1$ at $x_{1}$ and $x_{2}$
        \item Need to calculate the joint posterior, $p(\fV|\yV) = p(f_{1}, f_{2}| y_{1}=1, y_{2}=1)$.
        \item Requires 2D integral $p(y_1=1,y_2=1|f_1,f_2)p(f_1, f_2) df_1 df_2$.
    \end{itemize}
    \begin{overprint}
