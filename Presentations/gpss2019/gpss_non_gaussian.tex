...
\frame{
    \frametitle{Likelihood}
    $p(y|f)$ as a function of f
    MAKE PLOT OF LIKELIHOOD AS A FUNCTION OF F, SHOULD SHOW IT NOT NORMALISING
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Can normalise easily with numerical integration.
    \end{itemize}
    \begin{overprint}
        \onslide<1>\centerline{\includegraphics[width=0.8\textwidth]{../diagrams/1d_gp_prior_samples.pdf}}
        \onslide<2>\centerline{\includegraphics[width=0.8\textwidth]{../diagrams/1d_squashed_gp_prior_samples.pdf}}
        \onslide<3>\centerline{\includegraphics[width=0.8\textwidth]{../diagrams/1d_gp_post_samples.pdf}}
    \end{overprint}
}
...
\frame{
    \frametitle{Non-Gaussian posteriors illustrated}
    \begin{overprint}
        \centerline{\includegraphics[width=0.7\textwidth]{../diagrams/1d_marginals.pdf}}
    \end{overprint}
}
...
\frame{
    \frametitle{Non-Gaussian posteriors illustrated}
    \begin{itemize}
        \item Now consider we have two observations, $y_{1} = 1$ and $y_{2} = 1$ at two input locations, $x_{1}$ and $x_{2}$
        \item Need to calculate the joint posterior, $p(\fV|\yV) = p(f_{1}, f_{2}| y_{1}=1, y_{2}=1)$.
    \end{itemize}
    MAKE A MORE RELAVENT PLOT OF A GAUSSIAN PROCESS WITH TWO INPUT LOCATIONS
}
...
\frame{
   ...
   IMAGE WITH TRUE NON-GAUSSIAN 2D POSTERIOR, SAMPLES TAKEN FROM THE TRUE POSTERIOR, AND A GAUSSIAN APPROXIMATION
}

\frame{
    \frametitle{Why do we want an analytical form?}
    We have said you can't get a nice analytical form because we can't figure out the normaliser. 
    Why is this a problem though? 
    What do we want to do with this posterior?
}

\frame{
    \frametitle{Non-Gaussian posterior approximation}
    \begin{itemize}
        \item Various methods to make a Gaussian approximation, $q(\fV) \approx {\color{intract}p(\fV|\yV)} = \gaussianDist{\fV}{\mathbf{\mu}=?}{\Cmat=?}$.
...
\frame{
    \frametitle{Approximation validity}
    WHY IS IT OKAY TO APPROXIMATE A GAUSSIAN PROCESS WITH A GAUSSIAN AT TRAINING POINTS ONLY?
    HOW DOES IT HELP US EVERYWHERE ELSE?
}
...
\frame{
    \frametitle{Parameters effect}
    ILLUSTRATION OF WHAT HAPPENS WHEN ADJUSTING THE MEAN AND COVARIANCE PARAMETERS TO THE CONDITIONAL GAUSSIAN
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item KL-method takes the latter
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Idea of Laplace approximation is to find the values of the quadratic function that match at the mode of the log posterior.
        \item It is an approximation of the true posterior, based on the curvature at a single point, the mode.
        \item Many of you will know this as a second-order Taylor expansion around the modal value.
...
\frame{
    \frametitle{Laplace approximation - algorithm}
    \begin{itemize}
...
\frame{
    ...
    MAKE A VISUALISATION OF THE LAPLACE ON THE BERNOULLI TO ILLUSTRATE WHERE IT FALLS DOWN.
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Minimize this divergence between the two distributions~\citep{Nickisch:class08}, with respect to $\muV$ and $\Cmat$ and other parameters.
    \end{itemize}
}
...
\frame{
    \frametitle{Optimisation illustration}
    SINCE YOU HAVE SEEN WHAT THE KL DIVERGENCE BETWEEN TWO DISTRIBUTIONS. CONSIDER WHAT WOULD HAPPEN IF YOU COULD OPTIMISE 
    ILLUSTRATION OF WHAT IS BEING OPTIMISED, MAYBE SEVERAL SLIDES ON THIS SIMILAR TO JAMES
}
...
\frame{
    \frametitle{KL validity}
    WHY IS IT OKAY TO DEFINE THE KL DIVERGENCE BETWEEN TWO INFINITE PROCESSES BASED ON ONLY TRAINING POINTS?
}
...
\frame{
    \frametitle{Sampling}
    ALTHOUGH NOT OUR FOCUS, WHY IS IT DIFFICUILT AND WHAT METHODS ARE CURRENTLY USED?

    HOW CAN SAMPLES (HOWEVER THEY ARE OBTAINED) BE USED TO COMPUTE QUANTITIES OF INTEREST?
}
...
\frame{
    \frametitle{EP vs KL}
    ILLUSTRATE ON MULTI-MODAL THE EFFECT OF USING KL pq vs KL qp, i.e. BISHOP PLOT
}
...
\frame{
    ...
    MANY TASKS REQUIRE NON-GAUSSIAN OBSERVATION MODELS
    SOMETIMES BESPOKE LIKELIHOODS NEED TO BE CONSTRUCTED
    NON-GAUSSIAN LIKELIHOODS CAUSE COMPLICATIONS IN OUR FRAMEWORK
    DIFFERENT WAYS TO DEAL WITH THE PROBLEM, MANY ARE BASED ON GAUSSIAN APPROXIMATIONS
